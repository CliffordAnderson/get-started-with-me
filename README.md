...
favicon.ico             the site mark, generated by scripts/make-favicon.py
favicon.svg             the same mark as vector, from the same constants
...
`favicon.ico` is the pair `drawPoint` puts on every scatter plot in the lessons: class 0
as a blue ring, class 1 as a brown square, on the paper colour. It is generated rather
than drawn by hand, so it stays tied to the palette:

```bash
python3 scripts/make-favicon.py
python3 scripts/make-favicon.py --preview /tmp   # blown-up PNGs to look at
```

It writes both files from one set of constants, so the vector and the bitmap cannot drift
apart. The `.ico` is rasterised by supersampling into 16, 32 and 48 pixel entries as
32-bit BMP, which every browser accepts; the `.svg` is 341 bytes and stays sharp wherever
a browser wants the mark larger than 48. Proportions follow `drawPoint` except the ring
stroke: `0.325r` comes to barely one pixel at 16 across and greys out, so the ring is
thickened and the square set a little smaller than the ring to match its visual weight.
Look at a preview after changing any of those constants; the geometry is chosen so the
two shapes clear each other on the diagonal, and small edits close that gap fast.

The palette is fixed rather than following `prefers-color-scheme`. The site has one theme,
and a favicon that changed colour would be the only part of it that did; the paper
background also keeps two dark inks legible on a dark tab strip, which a transparent
version would not.

Every page carries both links with relative hrefs, so the mark appears from `file://` as
...
Browsers that understand SVG take the second and the rest fall back to the first.
`check-site.py` fails if a page is missing either link, or if either file no longer
matches the script.
...
the icon link on every page, the generated favicon and JavaScript syntax. The second runs the shipped mathematical engines: all nine gradient
